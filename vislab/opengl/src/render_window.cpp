#include <vislab/opengl/render_window.hpp>
...
namespace vislab
{
    RenderWindowOpenGL::RenderWindowOpenGL(int width, int height, const char* title, bool fullScreen)
        : mWnd(NULL)
        , interactor(nullptr)
    {
        if (!initWindow(width, height, title, fullScreen))
            throw std::runtime_error("Window creation failed.");
        mOpenGL = std::make_unique<OpenGL>(mWnd);
    ...
    const OpenGL* RenderWindowOpenGL::getOpenGL() const { return mOpenGL.get(); }

    bool RenderWindowOpenGL::init()
    {
        return true;
    }

    bool RenderWindowOpenGL::createDevice(OpenGL* opengl)
    {
        return true;
    }

    bool RenderWindowOpenGL::createSwapChain(OpenGL* opengl)
    {
        return true;
    }

    void RenderWindowOpenGL::releaseDevice()
    {
    }

    void RenderWindowOpenGL::releaseSwapChain()
    {
    }

    int RenderWindowOpenGL::run()
    {
    ...
    void RenderWindowOpenGL::resize(int width, int height)
    {
        if (mOpenGL == nullptr)
            return;
        releaseSwapChain();
        mOpenGL->releaseSwapChain();
        mOpenGL->createSwapChain();
        createSwapChain(mOpenGL.get());
        glViewport(0, 0, width, height);
    }

    bool RenderWindowOpenGL::initWindow(int width, int height, const char* title, bool fullScreen)
    {
        if (glfwInit() == 0)
            return false;
        ...
        {
            static_cast<RenderWindowOpenGL*>(glfwGetWindowUserPointer(w))->keyCallback(w, key, scancode, action, mods);
        };
        ...
        {
            static_cast<RenderWindowOpenGL*>(glfwGetWindowUserPointer(w))->scrollCallback(w, xoffset, yoffset);
        };
        ...
        {
            static_cast<RenderWindowOpenGL*>(glfwGetWindowUserPointer(w))->cursorPositionCallback(w, xpos, ypos);
        };
        ...
        {
            static_cast<RenderWindowOpenGL*>(glfwGetWindowUserPointer(w))->mouseButtonCallback(w, button, action, mods);
        };
        ...
        {
            static_cast<RenderWindowOpenGL*>(glfwGetWindowUserPointer(w))->windowSizeCallback(w, width, height);
        };
    ...
    void RenderWindowOpenGL::keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
    {
        KeyState mKeyState     = {};
    ...
    void RenderWindowOpenGL::scrollCallback(GLFWwindow* window, double xoffset, double yoffset)
    {
    ...
    void RenderWindowOpenGL::cursorPositionCallback(GLFWwindow* window, double xpos, double ypos)
    {
        mMouseState.x = xpos;
        mMouseState.y = ypos;
        if (interactor && interactor->active)
    ...
    void RenderWindowOpenGL::mouseButtonCallback(GLFWwindow* window, int button, int action, int mods)
    {
    ...
    void RenderWindowOpenGL::windowSizeCallback(GLFWwindow* window, int width, int height)
    {
        resize(width, height);
    }
}
